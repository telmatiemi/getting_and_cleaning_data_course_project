...
#Renaming columns so we will have more descriptive column names
features[,3] <- features[,2]
features[,3] <- gsub("^t", "time_", features[,3])
features[,3] <- gsub("^f", "frequency_", features[,3])
features[,3] <- gsub("Gyro", "Gyroscope", features[,3])
features[,3] <- gsub("Acc", "Accelerometer", features[,3])
...
test_train_avg <- aggregate(test_train[,3:68], by = list(activity, subject), FUN = mean)
